...
For 0.6.0, only `TEXT` format is supported.
...
MatrixOne supports any `SELECT`, `UPDATE`, `DELETE` statement execution plan. For `INSERT` statement, only `INSERT INTO..SELECT` is supported in 0.6.0 version. `INSERT INTO...VALUES` is not supported yet.
