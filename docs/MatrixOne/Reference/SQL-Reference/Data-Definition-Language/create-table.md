...
!!! note
    MatrixOne currently only supports the default increment step size of 1; regardless of the initial value of the auto-increment column, each auto-increment is 1. Temporarily does not support setting the incremental step size.
...
3. In MatrixOne, although the syntax supports `set @@auto_increment_increment=n` to set the incremental step size, it does not take effect; currently, it supports setting the initial value `AUTO_INCREMENT=n` of the auto-increment column, but the step size Still defaults to 1.
