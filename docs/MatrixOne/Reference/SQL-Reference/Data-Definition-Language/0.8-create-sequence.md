...
- `NEXTVAL(sequence_name)`: Sets the current value to the next value in the sequence and returns it.
- `CURRVAL(sequence_name)`: Returns the current value in the sequence.
- `SETVAL(sequence_name, n [,b])`: Sets the current value in the sequence to n.
...
- `LASTVAL()`: Return the value of any sequence obtained by `NEXTVAL` in the current session; if the current value is set by `SETVAL` first, and then use `LASTVAL` to operate on the sequence, return `SETVAL` The current value of the setting.
