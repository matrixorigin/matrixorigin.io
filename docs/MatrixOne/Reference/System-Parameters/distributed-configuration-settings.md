...
In the *matrixone/etc/launch-with-proxy/* directory, there are four configuration files: *cn.toml*, *dn.toml*, *proxy.toml*, and *log.toml*. The parameters contained in these configuration files are explained below:
...
| port-base         | Starting port number used by "DN". Find an available port for internal services by incrementing from this base port number, continuously checking the next 20 ports | port-base = 19000       |
