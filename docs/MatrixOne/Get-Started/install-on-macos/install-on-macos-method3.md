...
=== "Stable Version Image(1.1.3 version)"

      ```bash
      docker pull matrixorigin/matrixone:1.1.3
      docker run -d -p 6001:6001 --name matrixone matrixorigin/matrixone:1.1.3
      ```

      If you are using the network in mainland China, you can pull the MatrixOne stable version image on Alibaba Cloud:

      ```bash
      docker pull registry.cn-shanghai.aliyuncs.com/matrixorigin/matrixone:1.1.3
      docker run -d -p 6001:6001 --name matrixone registry.cn-shanghai.aliyuncs.com/matrixorigin/matrixone:1.1.3
      ```
...
```bash
docker run -d -p 6001:6001 --name matrixone --privileged=true matrixorigin/matrixone:1.1.3
```
...
mo_ctl set_conf MO_CONTAINER_IMAGE="matrixorigin/matrixone:1.1.3" # Set image
