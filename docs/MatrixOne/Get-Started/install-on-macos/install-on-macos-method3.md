...
=== "Stable Version Image(3.0.6 version)"

      ```bash
      docker pull matrixorigin/matrixone:3.0.6
      docker run -d -p 6001:6001 --name matrixone matrixorigin/matrixone:3.0.6
      ```

      If you are using the network in mainland China, you can pull the MatrixOne stable version image on Alibaba Cloud:

      ```bash
      docker pull registry.cn-shanghai.aliyuncs.com/matrixorigin/matrixone:3.0.6
      docker run -d -p 6001:6001 --name matrixone registry.cn-shanghai.aliyuncs.com/matrixorigin/matrixone:3.0.6
      ```
...
```bash
docker run -d -p 6001:6001 --name matrixone --privileged=true matrixorigin/matrixone:3.0.6
```
...
mo_ctl set_conf MO_CONTAINER_IMAGE="matrixorigin/matrixone:3.0.6" # Set image
