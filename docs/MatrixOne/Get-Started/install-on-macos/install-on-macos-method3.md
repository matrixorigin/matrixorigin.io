...
=== "Stable Version Image(2.1.1 version)"

      ```bash
      docker pull matrixorigin/matrixone:2.1.1
      docker run -d -p 6001:6001 --name matrixone matrixorigin/matrixone:2.1.1
      ```

      If you are using the network in mainland China, you can pull the MatrixOne stable version image on Alibaba Cloud:

      ```bash
      docker pull registry.cn-shanghai.aliyuncs.com/matrixorigin/matrixone:2.1.1
      docker run -d -p 6001:6001 --name matrixone registry.cn-shanghai.aliyuncs.com/matrixorigin/matrixone:2.1.1
      ```
...
mo_ctl set_conf MO_CONTAINER_IMAGE="matrixorigin/matrixone:2.1.1" # Set image
