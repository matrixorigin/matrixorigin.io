...
=== "Stable Version Image(2.2.2 version)"

      ```bash
      docker pull matrixorigin/matrixone:2.2.2
      docker run -d -p 6001:6001 --name matrixone matrixorigin/matrixone:2.2.2
      ```

      If you are using the network in mainland China, you can pull the MatrixOne stable version image on Alibaba Cloud:

      ```bash
      docker pull registry.cn-shanghai.aliyuncs.com/matrixorigin/matrixone:2.2.2
      docker run -d -p 6001:6001 --name matrixone registry.cn-shanghai.aliyuncs.com/matrixorigin/matrixone:2.2.2
      ```
...
```bash
docker run -d -p 6001:6001 --name matrixone --privileged=true matrixorigin/matrixone:2.2.2
```
...
mo_ctl set_conf MO_CONTAINER_IMAGE="matrixorigin/matrixone:2.2.2" # Set image
