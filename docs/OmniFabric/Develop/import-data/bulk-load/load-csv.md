...
This document will guide you on how to import large amounts of *.csv* format data to OmniFabric.
...
Make sure you have already [Deployed and Launched standalone OmniFabric](../../../Get-Started/install-standalone-matrixone.md).
...
#### The data file is in the same machine with the OmniFabric server

1. Before executing `Load Data` in OmniFabric, the table needs to be created in advance. For now, the data file is required to be at the same machine with OmniFabric server, a file transfer is necessary if they are in separate machines.

2. Launch the MySQL Client in the OmniFabric local server for accessing the local file system.
...
#### The data file is in separate machines with the OmniFabric server

1. Before executing `LOAD DATA LOCAL` in OmniFabric, the table needs to be created in advance.
...
If you install OmniFabric by `docker`, the file system is inside the docker image by default. To work with local directory, you need to bind a local directory to the container. In the following example, the local file system path `~/tmp/docker_loaddata_demo/` is binded to the OmniFabric docker image, with a mapping to the `/ssb-dbgen-path` path inside the docker.
We will walk you through the whole process of loading data with OmniFabric 2.1.0 docker version in this example.
...
5. Create *lineorder_flat* tables in OmniFabric, and import the dataset into MatriOne:
