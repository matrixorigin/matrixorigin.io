...
This document will guide you to use the `source` command to import data into OmniFabric in batches.
...
This tutorial will guide you on migrating data from MySQL to OmniFabric using the `source` command.
...
#### 2. Import into OmniFabric

Import the whole table structures and data into OmniFabric.

1. Open a MySQL terminal and connect to OmniFabric.
2. Import the SQL file into OmniFabric by the `source` command.
...
OmniFabric v25.2.1.0 version already supports MySQL table creation statements, so you can smoothly migrate MySQL tables to OmniFabric. However, it should be noted that during the migration process, some keywords incompatible with MySQL, such as `engine=`, will be automatically ignored in OmniFabric and will not affect the migration of the table structure.
