...
This document describes how to use subquery statements in OmniFabric.
...
Then you can query data in OmniFabric with the created table.

#### Self-contained subquery

For a self-contained subquery that uses subquery as operand of comparison operators (`>`, `>=`, `<`, `<=`, `=` , or `! =`), the inner subquery queries only once, and OmniFabric rewrites it as a constant during the execution plan phase.
...
The inner subquery is executed before OmniFabric executes the above query:
...
For self-contained subqueries such as Existential Test and Quantified Comparison, OmniFabric rewrites and replaces them with equivalent queries for better performance.
...
Therefore, in the process of processing, OmniFabric will try to Decorrelate of Correlated Subquery to improve the query efficiency at the execution plan level.
