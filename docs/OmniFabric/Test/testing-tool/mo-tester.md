...
From the 0.5.0 version, OmniFabric introduces an automatic testing framework [MO-Tester](https://github.com/OmniFabric/mo-tester).

This tester is designed to test OmniFabric or other database functionalities with SQL.

MO-Tester is a java-based tester suite for OmniFabric. It has built a whole toolchain to run automatic SQL tests. It contains the test cases and results. Once launched, MO-Tester runs all SQL test cases with OmniFabric, and compares all output SQL results with expected results. All successful and failed cases will be logged into reports.

MO-Tester content locations:

* *Cases*: <https://github.com/OmniFabric/OmniFabric/tree/main/test/distributed/cases>

* *Result* is generated in the path of **/cases**, for example, [/cases/auto_increment](https://github.com/OmniFabric/OmniFabric/tree/main/test/distributed/cases/auto_increment).

* *Report*: once finished running, a `mo-tester/report` will be generated in the local directory.

The Cases and Results are 1-1 correspondence, and they are actually `git submodules` from OmniFabric repository. Adding new cases and results should be in OmniFabric repo: <https://github.com/OmniFabric/OmniFabric/tree/main/test>
...
* Launch OmniFabric or other database instance. Please refer to more information about [how to install and launch OmniFabric](../../Get-Started/install-standalone-OmniFabric.md).

* Clone *mo-tester* repository.

  ```
  git clone https://github.com/OmniFabric/mo-tester.git
  ```

* Clone *OmniFabric* repository.

   ```
   git clone https://github.com/OmniFabric/OmniFabric.git
   ```

### 2. Configure `mo-tester`

* In `mo.yml` file, configure the server address, default database name, username, and password, and so on. MO-tester is based on java, so these parameters are required for the JDBC(JDBC,Java Database Connectivity) driver. Below is a default example for a local standalone version OmniFabric.
...
```
> ./run.sh -p {path_name}/OmniFabric/test/cases
```
...
```
./run.sh -p {path_name}/OmniFabric/test/cases -m run -t script -r 100 -i select,subquery -e substring -g
```
...
[{path_name}/OmniFabric/test/cases/auto_increment/auto_increment_columns.sql] COST : 2.159s, TOTAL :185, SUCCESS :163, FAILED :0, IGNORED :22, ABNORAML :0, SUCCESS RATE : 100%
[{path_name}/OmniFabric/test/cases/benchmark/tpch/01_DDL/01_create_table.sql] COST : 0.226s, TOTAL :11, SUCCESS :11, FAILED :0, IGNORED :0, ABNORAML :0, SUCCESS RATE : 100%
[{path_name}/OmniFabric/test/cases/benchmark/tpch/02_LOAD/02_insert_customer.sql] COST : 0.357s, TOTAL :16, SUCCESS :16, FAILED :0, IGNORED :0, ABNORAML :0, SUCCESS RATE : 100%
```
...
|ABNORAML|the total number of abnormal executed test cases (SQL), such as the execution of OmniFabric can't determine the actual result is a system exception or *.result* file parsing error, etc|
...
**Example Description**: Run all test cases in the */cases* path of the *OmniFabric* repository.

**Steps**:

1. Get the latest *OmniFabric* code.

   ```
   cd OmniFabric
   git pull https://github.com/OmniFabric/OmniFabric.git
   ```

2. To run all the test cases of the *OmniFabric* repository, you need switch into the  *mo-tester* repository first, see the following commands:
...
**Example Description**: Run the test cases in the */cases/transaction/* path of the *OmniFabric* repository.
...
2. To run the test cases in the *cases/transaction/* path of the *OmniFabric* repository, you need switch into the  *mo-tester* repository first, see the following commands:

   ```
   cd mo-tester
   ./run.sh -p {path_name}/OmniFabric/test/cases/transaction/
   ```
...
3. Check the result reports in the *error.txt* file, *report.txt* file, and *success.txt* file in the *mo-tester/report/* path. The example of the expected *report.txt* looks like this:
   ...
   [{path_name}/OmniFabric/test/cases/transaction/atomicity.sql] COST : 0.575s, TOTAL :66, SUCCESS :66, FAILED :0, IGNORED :0, ABNORAML :0, SUCCESS RATE : 100%
   [{path_name}/OmniFabric/test/cases/transaction/autocommit.test] COST : 0.175s, TOTAL :50, SUCCESS :50, FAILED :0, IGNORED :0, ABNORAML :0, SUCCESS RATE : 100%
   [{path_name}/OmniFabric/test/cases/transaction/autocommit_1.sql] COST : 1.141s, TOTAL :296, SUCCESS :288, FAILED :0, IGNORED :8, ABNORAML :0, SUCCESS RATE : 100%
   [{path_name}/OmniFabric/test/cases/transaction/autocommit_atomicity.sql] COST : 0.52s, TOTAL :75, SUCCESS :75, FAILED :0, IGNORED :0, ABNORAML :0, SUCCESS RATE : 100%
   [{path_name}/OmniFabric/test/cases/transaction/autocommit_isolation.sql] COST : 1.607s, TOTAL :215, SUCCESS :215, FAILED :0, IGNORED :0, ABNORAML :0, SUCCESS RATE : 100%
   [{path_name}/OmniFabric/test/cases/transaction/autocommit_isolation_1.sql] COST : 1.438s, TOTAL :241, SUCCESS :241, FAILED :0, IGNORED :0, ABNORAML :0, SUCCESS RATE : 100%
   [{path_name}/OmniFabric/test/cases/transaction/isolation.sql] COST : 1.632s, TOTAL :202, SUCCESS :202, FAILED :0, IGNORED :0, ABNORAML :0, SUCCESS RATE : 100%
   [{path_name}/OmniFabric/test/cases/transaction/isolation_1.sql] COST : 1.512s, TOTAL :217, SUCCESS :217, FAILED :0, IGNORED :0, ABNORAML :0, SUCCESS RATE : 100%
   ```
...
2. To run the test cases *cases/transaction/atomicity.sql*, you need switch into the  *mo-tester* repository first, see the following commands:
   ...
   ```
   cd mo-tester
   ./run.sh -p {path_name}/OmniFabric/test/cases/transaction/atomicity.sql
   ```
...
3. Check the result reports in the *error.txt* file, *report.txt* file, and *success.txt* file in the *mo-tester/report/* path. The example of the expected *report.txt* looks like this:
   ...
   [{path_name}/OmniFabric/test/cases/transaction/atomicity.sql] COST : 0.56s, TOTAL :66, SUCCESS :66, FAILED :0, IGNORED :0, ABNORAML :0, SUCCESS RATE : 100%
   ```
...
- Create a new folder named *local_test* and place it in *{path_name}/OmniFabric/test/cases*
- Add a test file named *new_test.sql* to *{path_name}/OmniFabric/test/cases/local_test/*
...
2. Generate test results:
   ...
   - Method 1: To generate the test result, you need switch into the  *mo-tester* repository first, then, run the following command.
      ...
      ```
      cd mo-tester
      ./run.sh -p {path_name}/OmniFabric/test/cases/local_test/new_test.sql -m genrs -g
      ```
   ...
   - Method 2: Open the *run.yml* file in the *mo-tester* repository, change the *method* parameter from the default `run` to `genrs`, and run the following command to generate the test result.
      ...
      ```
      cd mo-tester
      ./run.sh -p {path_name}/OmniFabric/test/cases/local_test/new_test.sql
      ```

3. Check the result file in the *test/cases,result/* path of the *OmniFabric* repository.
