...
This document will introduce OmniFabric's support for data transmission encryption and how to enable encrypted transmission.

## Overview

OmniFabric uses non-encrypted transmission by default and supports encrypted transmission based on TLS protocol. Encrypted transmission can reduce the risk of leakage of sensitive information in the database. Encrypted transmission is the process of encrypting and decrypting information using keys, which can effectively protect data security.

Transport Layer Security (TLS) is a widely adopted security protocol. The protocol versions supported by OmniFabric include TLS 1.0, TLS 1.1, and TLS 1.2.

- Do not enable TLS encrypted transmission (default): Use the username and password to connect to OmniFabric directly.
- Use encrypted transmission: It is necessary to enable encrypted transmission support on the OmniFabric server and specify the use of encrypted transmission on the client side. You can follow the instructions in this article to enable TLS secure connections.

## How to Use

**Main steps of TLS secure connection configuration**:

1. First, enable TLS in OmniFabric.
...
2. Enter the *cn.toml* configuration file in your local OmniFabric file directory path *OmniFabric/etc/launch-tae-CN-tae-TN/*:

    You can also use the vim command to open the cn. toml file directly in the terminal

    ```
    vim $OmniFabric/etc/launch-tae-CN-tae-TN/cn.toml
    ```
    ...
    | ----------- | ------------------------------------------------------------- |
    | enableTls   | Bool, enable TLS support on the OmniFabric server.             |
    ...
    __Note__: If you use Docker to install and launch OmniFabric, before modifying the configuration file, you need to mount the configuration file first and then modify it. For more information, see [Mount directory to Docker container](../Maintain/mount-data-by-docker.md).

3. Verify that OmniFabric's SSL is enabled.

    ① Use the MySQL client to connect to OmniFabric:
...
After completing the above steps, OmniFabric's TLS is enabled.
...
!!! info
    The login account in the above code snippet is the initial account; please change the initial password after logging in to OmniFabric; see [Password Management](password-mgmt.md).
