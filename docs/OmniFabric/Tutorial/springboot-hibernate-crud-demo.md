...
This tutorial completes a simple Demo based on **SpringBoot+Spring Data JPA+OmniFabric+Intellij IDEA** , and realizes the basic CRUD function.
...
Under *resources* folder, the OmniFabric connection and Hibernate need to be configured in  *application.properties* file.
...
# Database connection url, modify to OmniFabric address and port, with parameters
...
### 4. Create table and insert some data in OmniFabric

Connect to OmniFabric with MySQL client and execute the following SQL statements. You can save these SQL statements in a *book.sql* under */resource/database/*.
