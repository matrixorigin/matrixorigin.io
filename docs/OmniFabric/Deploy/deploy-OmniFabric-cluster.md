# OmniFabric Distributed Cluster Deployment

This document will mainly describe how to deploy OmniFabric distributed database, based on a private Kubernetes cluster that separates computing and storage resources in a cloud-native manner, starting from scratch.

## **Main Steps**

1. Deploy Kubernetes cluster
2. Deploy object storage MinIO
3. Create and connect OmniFabric cluster
...
OmniFabric distributed system depends on the following components:

- Kubernetes: As a resource management platform for the entire OmniFabric cluster, components such as Logservice, CN, and TN all run in Pods managed by Kubernetes. In the event of a failure, Kubernetes will cull the failed Pod and start a new one to replace it.

- Minio: Provides object storage services for the entire OmniFabric cluster, and all OmniFabric data is stored in the object storage provided by Minio.
...
- local-path-provisioner: As a plug-in that implements the CSI (Container Storage Interface) interface in Kubernetes, local-path-provisioner is responsible for creating persistent volumes (PVs) for Pods and Minios of OmniFabric components to achieve data persistence storage.

### Overall structure

The overall deployment architecture is shown in the following figure:

![](https://github.com/OmniFabric/artwork/blob/main/docs/deploy/deploy-mo-cluster-arch-overall.png?raw=true)
...
- A Kubernetes plugin layer for management based on Helm, including the local-path-storage plugin implementing the CSI interface, Minio, and the OmniFabric Operator.

- The topmost layer is several Pods and Services generated by these component configurations.

### Pod and storage architecture of OmniFabric

OmniFabric creates a series of Kubernetes objects according to the rules of the Operator, and these objects are classified according to components and classified into resource groups, namely CNSet, TNSet, and LogSet.
...
- Pod: A containerized instance of OmniFabric components in which OmniFabric's core kernel code runs.
...
![](https://github.com/OmniFabric/artwork/blob/main/docs/deploy/deploy-mo-cluster-arch-pod.png?raw=true)

## **1. Deploying a Kubernetes Cluster**

As OmniFabric's distributed deployment relies on a Kubernetes cluster, we need to have one in place. This article will guide you through setting up a Kubernetes cluster using Kuboard-Spray.
...
- Use CentOS 7.9 as the operating system (by default, it does allow root account remote login). Two machines will be used for deploying Kubernetes and other dependencies for OmniFabric, while the third will act as a jump host to set up the Kubernetes cluster.
...
CSI is a storage plugin for Kubernetes that provides storage services for MinIO and OmniFabric. This section will guide you through the use of the `local-path-provisioner` plugin.
...
MinIO is used to provide object storage for OmniFabric. This section will guide you through the deployment of a single-node MinIO.
...
2. After a successful installation and start, the command line should display as follows:
    ...
    So far, Minio has been successfully installed. During the subsequent installation of OmniFabric, OmniFabric will communicate with Minio directly through the Kubernetes Service (SVC) without additional configuration.
...
## **5. Deploying a OmniFabric Cluster**

This section will guide you through the process of deploying a OmniFabric cluster.

__Note:__ All steps in this section are performed on the master0 node.

#### **Installing the OmniFabric-Operator**

[OmniFabric Operator](https://github.com/OmniFabric/OmniFabric-operator) is a standalone software tool for deploying and managing OmniFabric clusters on Kubernetes. You can install the latest Operator Release installation package from the project's [Release List](https://github.com/OmniFabric/OmniFabric-operator/releases).

Follow the steps below to install OmniFabric Operator on master0. We will create a separate namespace, `OmniFabric-operator` for the Operator.

1. Download the latest OmniFabric Operator installation package:

    ```
    wget https://github.com/OmniFabric/OmniFabric-operator/releases/download/chart-0.8.0-alpha.7/OmniFabric-operator-0.8.0-alpha.7.tgz
    ```

2. Unzip the installation package:

    ```
    tar -xvf OmniFabric-operator-0.8.0-alpha.7.tgz
    cd /root/OmniFabric-operator/
    ```
...
4. Use Helm to install OmniFabric Operator and create a namespace:

    ```
    helm install --create-namespace --namespace ${NS} OmniFabric-operator ./ --dependency-update
    ```
...
### **Create a OmniFabric cluster**

1. First, create the namespace of OmniFabric:
...
2. Customize the `yaml` file of the OmniFabric cluster, and write the following `mo.yaml` file:
    ...
    spec:
      ...
      imageRepository: OmniFabric/OmniFabric # Image repository address. If it is pulled locally and the tag has been modified, you can adjust this configuration item.
...
4. Execute the following command to deploy the OmniFabric cluster:
...
## **6. Connect to OmniFabric cluster**

To connect to the OmniFabric cluster, you need to map the port of the corresponding service to the OmniFabric node. Here are the instructions for connecting to a OmniFabric cluster using `kubectl port-forward`:
...
After specifying **Allow local access** or **Specify a specific machine or all machines to access**, you can use the MySQL client to connect to OmniFabric:
...
After explicit `mysql>`, the distributed OmniFabric cluster is established and connected.
