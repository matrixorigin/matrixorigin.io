# Deploy a stand-alone OmniFabric based on S3

This article provides an example of how to deploy a stand-alone OmniFabric based on S3. Different from the general stand-alone deployment method, this method uses S3 as the storage medium of OmniFabric, instead of directly using the local disk of the host machine.
...
Prepare a host machine and configure S3 storage media, including but not limited to: authentication key pair, access endpoint, area (privatization can be ignored), bucket name path, etc. In addition, it is recommended to place the host of OmniFabric stand-alone deployment and S3 object storage in the same intranet to ensure that the network will not become a performance bottleneck.

## Operation steps

### Scenario 1: OmniFabric with Minio

#### Deploy a stand-alone OmniFabric

For reference [Deploy OmniFabric on a stand-alone basis](../Get-Started/install-standalone-OmniFabric.md), complete the deployment of the stand-alone version of OmniFabric, and do not start it after deployment.

#### Modifying the OmniFabric Configuration File
...
[[fileservice]]
backend = "DISK"
data-dir = "/your/mo-path/OmniFabric/data/local/cn"
name = "LOCAL"
...
disk-path = "/your/mo-path/OmniFabric/data/disk-cache" # Local Disk Cache Path
...
[[fileservice]]
backend = "DISK"
data-dir = "/your/mo-path/OmniFabric/data/local/tn"
name = "LOCAL"
...
[[fileservice]]
backend = "DISK"
data-dir = "/your/mo-path/OmniFabric/data/local/log"
name = "LOCAL"
...
#### Verify that OmniFabric is working properly

```
# Take the example of creating a library table and write some sample data to confirm that mo works properly
github@shpc2-10-222-1-9:/data/mo/main/OmniFabric/etc/launch$ mo_ctl connect
...
Server version: 8.0.30-OmniFabric-v287278 OmniFabric
...
```
github@shpc2-10-222-1-9:/data/mo/main/OmniFabric/etc/launch$ mc ls minio-sh/mo-on-minio-demo/mo
[2024-08-26 18:00:10 CST]     0B data/
github@shpc2-10-222-1-9:/data/mo/main/OmniFabric/etc/launch$ mc ls minio-sh/mo-on-minio-demo/mo/data/
...
mo_ctl set_conf MO_CONTAINER_IMAGE=OmniFabric/OmniFabric/2.1.0 #Setting up mirroring
...
- Start the OmniFabric container

Set the OmniFabric deployment mode to container and start the OmniFabric container
