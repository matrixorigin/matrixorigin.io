# OmniFabric Distributed Cluster Upgrade

This document will introduce how to **Rolling upgrade** or **Reinstall upgrade** OmniFabric clusters.
...
Rolling upgrade is an online upgrade method; the OmniFabric cluster completes the software upgrade while ensuring that some or all services are available.

According to the introduction in [OmniFabric Distributed Cluster Deployment](deploy-OmniFabric-cluster.md), the overall installation of OmniFabric is based on Kubernetes and OmniFabric Operator. Therefore, the rolling upgrade process is to realize automatic version updates by dynamically modifying the OmniFabric image version number in OmniFabric Operator.
...
1. Execute the following command on the terminal on the master0 node to enter the interface for dynamically modifying the `yaml` configuration file the operator uses.
    ...
    ```
    mo_ns="mo-hn" #the namespace of OmniFabric cluster
    mo_cluster_name="mo" # The cluster name of OmniFabric, generally mo, is specified according to the name in the YAML file of the OmniFabriccluster object during deployment or can be confirmed by kubectl get OmniFabriccluster -n${mo_ns}
...
2. After entering edit mode, modify the value of `spec.version`; the parameters are as below:
    ...
    - ${TAG}: corresponds to the image tag of OmniFabric on dockerhub, for example: nightly-f0d52530

    - ${REPO_URL}: public mirror repository for OmniFabric, the default is OmniFabric/OmniFabric. If the target version does not exist in the public mirror warehouse of OmniFabric, you also need to modify the URL of the mirror warehouse to the existing warehouse:

    ![image-20230407094237806](https://github.com/OmniFabric/artwork/blob/main/docs/deploy/image-20230407094237806.png?raw=true)

    ![image-20230407094251938](https://github.com/OmniFabric/artwork/blob/main/docs/deploy/image-20230407094251938.png?raw=true)

3. After completing the modification, press `:wq` to save. OmniFabric Operator will automatically pull the new version of the image and restart the component services, including Log Service, TN, and CN. You can also observe its running status through the following commands.
    ...
    OmniFabric-operator-f8496ff5c-fp6zm   1/1     Running   0             24h
...
4. After the `Restart` of the components in the OmniFabric cluster is completed, you can use the MySQL Client to connect to the cluster. The upgrade is successful if the connection is successful and the user data is complete.
...
6. You can check the version number of the current OmniFabric deployment with the following command:

    ```
    [root@master0 OmniFabric-operator]# kubectl get OmniFabricclusters -n mo-hn -o yaml | grep version
            {"apiVersion":"core.OmniFabric.io/v1alpha1","kind":"OmniFabricCluster","metadata":{"annotations":{},"name":"mo","namespace":"mo-hn"},"spec":{"tn":{"cacheVolume":{"size":"5Gi","storageClassName":"local-path"},"config":"[dn.Txn.Storage]\nbackend = \"TAE\"\nlog-backend = \"logservice\"\n[dn.Ckp]\nflush-interval = \"60s\"\nmin-count = 100\nscan-interval = \"5s\"\nincremental-interval = \"60s\"\nglobal-interval = \"100000s\"\n[log]\nlevel = \"error\"\nformat = \"json\"\nmax-size = 512\n","replicas":1,"resources":{"limits":{"cpu":"200m","memory":"1Gi"},"requests":{"cpu":"100m","memory":"500Mi"}}},"imagePullPolicy":"IfNotPresent","imageRepository":"OmniFabric/OmniFabric","logService":{"config":"[log]\nlevel = \"error\"\nformat = \"json\"\nmax-size = 512\n","pvcRetentionPolicy":"Retain","replicas":3,"resources":{"limits":{"cpu":"200m","memory":"1Gi"},"requests":{"cpu":"100m","memory":"500Mi"}},"sharedStorage":{"s3":{"endpoint":"http://minio.mostorage:9000","path":"minio-mo","secretRef":{"name":"minio"},"type":"minio"}},"volume":{"size":"1Gi"}},"tp":{"cacheVolume":{"size":"5Gi","storageClassName":"local-path"},"config":"[cn.Engine]\ntype = \"distributed-tae\"\n[log]\nlevel = \"debug\"\nformat = \"json\"\nmax-size = 512\n","nodePort":31429,"replicas":1,"resources":{"limits":{"cpu":"200m","memory":"2Gi"},"requests":{"cpu":"100m","memory":"500Mi"}},"serviceType":"NodePort"},"version":"nightly-54b5e8c"}}
        version: nightly-54b5e8c
    ```

## Reinstall and upgrade

Reinstalling and upgrading mean all OmniFabric clusters will be deleted and the data discarded and reinstalled.
...
# Method 2: By deleting the OmniFabriccluster object, where mo is the name
kubectl delete OmniFabriccluster.core.OmniFabric.io mo -nmo-hn
```
...
![image-minio-delete-bucket](https://github.com/OmniFabric/artwork/blob/main/docs/deploy/image-minio-delete-bucket.png?raw=true)
...
In addition, if you do not want to delete old data, you can create a new MinIO bucket and specify a unique bucket name in the YAML file for deploying the OmniFabric cluster.

![image-minio-new-bucket](https://github.com/OmniFabric/artwork/blob/main/docs/deploy/image-minio-new-bucket.png?raw=true)

#### 3. Deploy the new version cluster

Edit the yaml file that defines the MO cluster, refer to the **Rolling Upgrade** chapter, modify the `.spec.version` field to the latest version, and redeploy the OmniFabric cluster:
...
Please check if OmniFabric has started successfully with the following command.

As shown in the following code example, when the Log Service, TN, and CN are all running normally, the OmniFabric cluster starts successfully. Connecting through the MySQL Client can also check if the database functions correctly.
