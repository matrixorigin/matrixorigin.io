...
| Decimal256  | 32 bytes | 	65 digits  |  Decimal(N,S) <br> N is the total number of digits, the range is(38 ~ 65). The decimal point and (for negative numbers) the - sign are not counted in N.<br>If N is omitted, the default value of N should be the largest; that is, the value is 65. <br>S is the number of digits after the decimal point (the scale), the range is(0 ~ N)<br> If S is 0, values have no decimal point or fractional part. If S is omitted, the default is 0, for example, Decimal(40) is equivalent to Decimal(40, 0).  |
